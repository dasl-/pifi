...
## Setting your youtube API key
Use the script: https://github.com/dasl-/pifi/blob/main/utils/youtube_api_key[`./utils/youtube_api_key`], run through the pifi venv (created by `./install/install_dependencies.sh`):
```
sudo ./.venv/bin/python ./utils/youtube_api_key --api-key <your youtube API key>
```
